...
def move_expired_messages_to_archive(realm, dry_run=False):
...
def move_expired_user_messages_to_archive(realm, dry_run=False):
    # type: (bool) -> Optional[List[Tuple[Any]]]
    select_query_template = """
        SELECT {src_fields}, '{archive_timestamp}'
...
def archive_messages(dry_run=False):
    # type: (bool) -> Optional[List[Dict[str, int]]]
...
def restore_archived_attachments_message_rows_by_realm(realm_id, dry_run=False):
    ...
    select_query_template = """
       ...
       {check_query_part}
       INNER JOIN zerver_archivedusermessage ON zerver_archivedusermessage.message_id = zerver_archivedattachment_messages.archivedmessage_id
       INNER JOIN zerver_userprofile ON zerver_archivedusermessage.user_profile_id = zerver_userprofile.id
       LEFT JOIN zerver_attachment_messages ON zerver_archivedattachment_messages.id = zerver_attachment_messages.id 
       WHERE zerver_userprofile.realm_id = {realm_id}
